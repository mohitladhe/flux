...
export function DetailsPanel({ conversation, open, onClose }) {
  return (
    ...
    >
      <div
        className={`flex h-full w-[min(22rem,88vw)] flex-col transition-transform duration-300 ease-out ${
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          <div className="rounded-2xl border app-card p-5 text-center">
            <Avatar
              ...
              size="lg"
            />
            <h3 className="mt-4 text-xl font-bold app-text">{conversation.name}</h3>
            <p className="mt-1 text-sm app-muted">{conversation.members.length} members</p>
          ...
          <div className="mt-6">
            ...
            {["production-checklist.enc", "device-fingerprints.enc"].map((file) => (
              <button
                key={file}
                type="button"
